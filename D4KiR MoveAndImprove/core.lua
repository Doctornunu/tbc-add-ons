...
function MAICheckIfSame(src, dest)
	...
	for i, v in pairs(src) do
		if src[i] ~= dest[i] and i ~= "ILVL" and i ~= "XPTAB" and i ~= "menu" and i ~= "toggler" then
			if type(src[i]) == "number" then
...
function MAICreateSlider(parent, name, key, vval, x, y, vmin, vmax, steps, form, lstr)
	...
	SL:SetWidth(parent:GetWidth() - 20)
...
function MAIMoveFrames()
	if not InCombatLockdown() then
			...
			if frame and frameinit[name] == nil then
				...
				if MAIGV( name .. "Enabled" ) then
					...
					frame:SetScript( "OnDragStart", function(self, btn)
						...
						elseif IsShiftKeyDown() and btn == "LeftButton" then
							if not InCombatLockdown() then
								self:StartMoving()
							end
					...
					if not dontscale and MAIGV( name .. "Scale" ) then
						local resizeButton = CreateFrame("Button", "resButton", frame)
						...
						resizeButton:SetScript("OnUpdate", function(self, button)
							if self.isScaling == true then
								if frame.x == nil and frame:GetLeft() then
									frame.x = frame:GetLeft() 
									frame.y = (frame:GetTop() - frame:GetHeight()) 
								end
								...
								if newscale then
									...
									MAISV( name .. "scale", newscale )

									local tx, ty = self:GetParent().x / newscale, self:GetParent().y / newscale
		...
		if not allsetup then
			C_Timer.After(0.5, MAIMoveFrames)
		end
	else
...
function MAISetupMAIMENU()
	local Y = -10
	MAIMENUMOVING:SetSize(bbr + btnw + sbr + btnw + bbr, 120 + 140 + 50 + 30)
	...
	MAIMENUMOVING.r = MAICreateSlider(MAIMENUMOVING, "MAIMENUMOVING.r", "MAIUIColorR", MAIGV( "MAIUIColorR" ), 10, -150, 0, 1, 0.01, ".2f", MAIGT("uicolor") .. " " .. "|cFFFF0000" .. "Red")
	MAIMENUMOVING.r:HookScript("OnValueChanged", function(self, val)
		MAISV( "MAIUIColorR", MAIMathR(val, 2) )
		MAIUIColorUpdate()
	end)
	MAIMENUMOVING.g = MAICreateSlider(MAIMENUMOVING, "MAIMENUMOVING.g", "MAIUIColorG", MAIGV( "MAIUIColorG" ), 10, -190, 0, 1, 0.01, ".2f", MAIGT("uicolor") .. " " .. "|cFF00FF00" .. "Green")
	MAIMENUMOVING.g:HookScript("OnValueChanged", function(self, val)
		MAISV( "MAIUIColorG", MAIMathR(val, 2) )
		MAIUIColorUpdate()
	end)
	MAIMENUMOVING.b = MAICreateSlider(MAIMENUMOVING, "MAIMENUMOVING.b", "MAIUIColorB", MAIGV( "MAIUIColorB" ), 10, -230, 0, 1, 0.01, ".2f", MAIGT("uicolor") .. " " .. "|cFF0000FF" .. "Blue")
	MAIMENUMOVING.b:HookScript("OnValueChanged", function(self, val)
		MAISV( "MAIUIColorB", MAIMathR(val, 2) )
		MAIUIColorUpdate()
	end)

	MAIMENUMOVING.ZoomOut = MAICreateSlider(MAIMENUMOVING, "MAIMENUMOVING.ZoomOut", "MAIZoomOut", MAIGV( "ZoomOut" ), 10, -280, 1.0, maxzoom, 0.1, ".1f", MAIGT("zoomout"))
	MAIMENUMOVING.ZoomOut:HookScript("OnValueChanged", function(self, val)
		MAISV( "ZoomOut", MAIMathR(val, 2) )
		ConsoleExec("cameraDistanceMaxZoomFactor " .. MAIGV( "ZoomOut" ))
	end)

	MAIMENUMOVING.WorldTextScale = MAICreateSlider(MAIMENUMOVING, "MAIMENUMOVING.WorldTextScale", "MAIWorldTextScale", MAIGV( "WorldTextScale" ), 10, -310, 0.1, 10.0, 0.1, ".1f", MAIGT("WorldTextScale"))
	MAIMENUMOVING.WorldTextScale:HookScript("OnValueChanged", function(self, val)
		MAISV( "WorldTextScale", MAIMathR(val, 2) )
		ConsoleExec("WorldTextScale " .. MAIGV( "WorldTextScale" ))
	end)

	if MAIGV( "nochanges" ) then
		MAIMENUMOVING.r:Disable()
		MAIMENUMOVING.r.Text:SetTextColor( 0.4, 0.4, 0.4, 1 )
		MAIMENUMOVING.g:Disable()
		MAIMENUMOVING.g.Text:SetTextColor( 0.4, 0.4, 0.4, 1 )
		MAIMENUMOVING.b:Disable()
		MAIMENUMOVING.b.Text:SetTextColor( 0.4, 0.4, 0.4, 1 )
		MAIMENUMOVING.ZoomOut:Disable()
		MAIMENUMOVING.ZoomOut.Text:SetTextColor( 0.4, 0.4, 0.4, 1 )
		MAIMENUMOVING.WorldTextScale:Disable()
		MAIMENUMOVING.WorldTextScale.Text:SetTextColor( 0.4, 0.4, 0.4, 1 )
	end
	...
	MAIMENU:SetSize(bbr + btnw + sbr + btnw + bbr, 296)
	...
	MAIMENU.TOGGLE:SetPoint("TOPLEFT", MAIMENU, "TOPLEFT", bbr + btnw + sbr, py)
	...
	MAIMENU.FRAMES:SetPoint("TOPLEFT", MAIMENU, "TOPLEFT", bbr, py)
	...
	MAIMENU.nochanges.text:SetFont(STANDARD_TEXT_FONT, 8, "")
	...
	MAIMENU.showilvl:SetScript("OnUpdate", function(self)
	...
	MAIINFO.text:SetFont(STANDARD_TEXT_FONT, 36, "THINOUTLINE")
...
function MAIElementSetup(element)
	if MAILoaded and MAIGV( element.name .. "Enabled" ) then
		if _G[element.name] == nil then
			...
			mover.settingsbtn:SetText(strsub(getglobal("SETTINGS"), 0, 1))
			...
			mover.settingsbtn:SetScript("OnClick", function(self)
				if mover.settings == nil then
					...
					mover.tab1.content.up:SetPoint("TOPLEFT", 30, -20)
					...
					mover.tab1.content.dn:SetPoint("TOPLEFT", 30, -60)
					...
					mover.tab1.content.ri:SetPoint("TOPLEFT", 50, -40)
					...
					mover.tab1.content.li:SetPoint("TOPLEFT", 10, -40)
					...
					mover.textsca:SetPoint("LEFT", mover.tab1.content, "TOPLEFT", 100, -10)
					...
					mover.tab1.content.scaleup:SetPoint("TOPLEFT", 100, -20)
					...
					mover.tab1.content.scaleup:SetSize(20, 20)
					mover.tab1.content.scaleup:SetPoint("TOPLEFT", 100, -40)
					...
					mover.tab1.content.move:SetPoint("TOPLEFT", 10, -100)
					...
					mover.tab1.content.improvements:SetPoint("TOPLEFT", 10, -120)
					...
					mover.tab1.content.hideartwork:SetPoint("TOPLEFT", 10, -140)
...
local MAIGlowAlpha = 0.5

function MAIAddIlvl(SLOT)
	if SLOT and SLOT.text == nil then
		...
		SLOT.text:SetPoint("TOPLEFT", SLOT, "TOPLEFT", 3, -3)

		SLOT.texth = SLOT:CreateFontString(nil, "ARTWORK")
		SLOT.texth:SetFont(STANDARD_TEXT_FONT, 9, "THINOUTLINE")
		SLOT.texth:SetShadowOffset(1, -1)
		SLOT.texth:SetPoint("BOTTOM", SLOT, "BOTTOM", 0, 3)

		SLOT.border = SLOT:CreateTexture(nil, "OVERLAY", SLOT)
...
function MAISetup()
	...
	function MAIAddStanceButton(spell1, spell2, hr, allclassesplus, nl)
		...
		if spellid1 == nil and art1 == nil then
			...
			if MAIGV( "StanceBar" .. "Enabled" ) and MAIGV( "StanceBar" .. "allclasses" ) then
				...
				b.NormalTexture2:SetVertexColor(MAIGetUIColor())
	...
	elseif englishClass == "SHAMAN" then
		...
		if MAIGV( "CLASSES" )["SHAMANVERSION"] ~= 5 then
			MAIGV( "CLASSES" )["SHAMANVERSION"] = 5
	...
	elseif englishClass == "WARLOCK" then
		...
		if MAIGV( "CLASSES" )["WARLOCKVERSION"] ~= 3 then
			...
			MAIGV( "CLASSES" )["WARLOCK"][1][4] = {691, "daemon"} -- Teufelsjäger
			MAIGV( "CLASSES" )["WARLOCK"][1][5] = {1122, "daemon"} -- Inferno
...
function MAIUnitXP(unit)
	...
	if MAILoaded and UnitExists(unit) and MAIGV( "XPTAB" ) and MAIGV( "XPTAB" )[target] and MAIGV( "XPTAB" )[target]["useapi"] then
...
function MAIUpdatePartyXPAPI()
	if MAILoaded then
		for i = 1, 4 do
			...
			MAIGV( "XPTAB" )[target]["useapi"] = false
		end
			...
			if isInitialLogin or isReloadingUi then
				...
				for i = 1, 4 do
					...
					if PartyFrame then
						...
						PartyFrameXPBar:SetPoint("TOP", ManaBar, "BOTTOM")
				
						PartyFrameXPBar.texture = PartyFrameXPBar:CreateTexture(nil, "BACKGROUND")
						PartyFrameXPBar.texture:SetTexture("Interface\\TargetingFrame\\UI-StatusBar")
						PartyFrameXPBar.texture:SetSize(10, sh)
						PartyFrameXPBar.texture:SetPoint("LEFT", PartyFrameXPBar, "LEFT", 0, 0)
						PartyFrameXPBar.texture:SetColorTexture(0.25, 0.5, 1.0, 1.0)
				
						PartyFrameXPBar.texture2 = PartyFrameXPBar:CreateTexture(nil, "BORDER")
						PartyFrameXPBar.texture2:SetTexture("Interface\\Tooltips\\UI-StatusBar-Border")
						PartyFrameXPBar.texture2:SetSize(sw + 4, sh + 4)
						PartyFrameXPBar.texture2:SetPoint("CENTER", PartyFrameXPBar, "CENTER", 0, 0)
				
						PartyFrameXPBar.textl = PartyFrameXPBar:CreateFontString(nil, "ARTWORK", "TextStatusBarText") 
						PartyFrameXPBar.textl:SetFont(STANDARD_TEXT_FONT, 9, "")
						PartyFrameXPBar.textl:SetShadowOffset(1, -1)
						PartyFrameXPBar.textl:SetPoint("LEFT", PartyFrameXPBar, "LEFT", 2, 0)
				
						PartyFrameXPBar.textr = PartyFrameXPBar:CreateFontString(nil, "ARTWORK", "TextStatusBarText") 
						PartyFrameXPBar.textr:SetFont(STANDARD_TEXT_FONT, 9, "")
						PartyFrameXPBar.textr:SetShadowOffset(1, -1)
						PartyFrameXPBar.textr:SetPoint("RIGHT", PartyFrameXPBar, "RIGHT", -2, 0)
						
						PartyFrameXPBar.textc = PartyFrameXPBar:CreateFontString(nil, "ARTWORK") 
						PartyFrameXPBar.textc:SetFont(STANDARD_TEXT_FONT, 9, "")
						PartyFrameXPBar.textc:SetShadowOffset(1, -1)
						PartyFrameXPBar.textc:SetPoint("TOPLEFT", PartyFrameXPBar, "BOTTOMLEFT", 0, -1)
						...
						function PartyFrameXPBar.think()
							if UnitExists("PARTY" .. i) and UnitLevel("PARTY" .. i) < MAIGetMaxLevel() then
								PartyFrameXPBar.textc:SetText(UnitLevel("PARTY" .. i))

								if MAIUnitXPMax("PARTY" .. i) > 0 and not MAIGV( "nochanges" ) then
									PartyFrameXPBar:Show()
									local per = MAIUnitXP("PARTY" .. i) / MAIUnitXPMax("PARTY" .. i)
									PartyFrameXPBar.texture:SetWidth(per * PartyFrameXPBar:GetWidth() - 4)
					
									PartyFrameXPBar.textl:SetText(string.format("%0.1f", MAIUnitXP("PARTY" .. i) / MAIUnitXPMax("PARTY" .. i) * 100) .. "%")
									PartyFrameXPBar.textr:SetText(MAIFormatValue(MAIUnitXP("PARTY" .. i)))
								else
