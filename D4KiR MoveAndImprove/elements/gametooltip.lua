...
function MAISetupGameTooltip()
	...
	if MAIGV( "GameTooltip" .. "improvements" ) and MAIBUILD ~= "RETAIL" and not MAIGV( "nochanges" ) then
		local maiclassids = {
			2, -- weapon
			3, -- gem
			4, -- armor
		...
		local function ItemExpansion(tt)
			...
			local itemName, itemLink, itemQuality, itemLevel, itemMinLevel, itemType, itemSubType, itemStackCount, itemEquipLoc, itemTexture, price, classID, _, _, _, _, _ = GetItemInfo( itemId )
