...
function MAIZoomBuffs(id)
	if MAIGV( "MAIBuffFrame" .. "Enabled" ) and MAIGV( "MAIBuffFrame" .. "improvements" ) and not MAIGV( "nochanges" ) then
		...
		if border ~= nil and border.hooked == nil then
			...
			border:SetVertexColor(MAIGetUIColor())
				...
				if ( numBuffs == 1 ) then
					if ( enchants > 0 and TemporaryEnchantFrameMover == nil) then
						...
						if DIRB == "LEFT" then
							...
							buff.t2 = parent
							buff.t3 = "TOPRIGHT"
							buff.t4 = -enchants * (buff:GetHeight() + MAIGV( "MAIBuffFrame" .. "spacing" ))
							buff.t5 = 0
						else
							buff.t1 = "TOPLEFT"
							buff.t2 = parent
		...
		hooksecurefunc(MAIBuffFrameMover, "SetPoint", function(self, ...)
			if oldenchants ~= enchants then
				...
				if TempEnchant3 ~= nil then
					...
					if DIRT == "LEFT" then
						TempEnchant3:SetPoint("TOPRIGHT", TempEnchant2, "TOPRIGHT", -(TempEnchant2:GetWidth() + MAIGV( "MAIBuffFrame" .. "spacing" )), 0)
					else
						TempEnchant3:SetPoint("TOPLEFT", TempEnchant2, "TOPLEFT", TempEnchant2:GetWidth() + MAIGV( "MAIBuffFrame" .. "spacing" ), 0)
					end
				end
