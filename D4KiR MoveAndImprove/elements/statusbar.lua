...
function MAISetupStatusBar()
	...
	for i = 0, 90, 10 do
		MAIStatusBar.XPBar["overlaygrid" .. i]:SetVertexColor(MAIGetUIColor())
		MAIRegisterUIColor(MAIStatusBar.XPBar["overlaygrid" .. i])
	end
	for i = 0, 90, 10 do
		MAIStatusBar.REPBar["overlaygrid" .. i]:SetVertexColor(MAIGetUIColor())
		MAIRegisterUIColor(MAIStatusBar.REPBar["overlaygrid" .. i])
	end
	for i = 0, 90, 10 do
		MAIStatusBar.XPBarPet["overlaygrid" .. i]:SetVertexColor(MAIGetUIColor())
		...
		if not MAIIsXPBarVisible() then
			if not InCombatLockdown() then
				if self.pos ~= 1 then
					...
					self:SetPoint("TOPLEFT", MAIStatusBar, "TOPLEFT", 0, 0)
				end
			end
		else
			if not InCombatLockdown() then
	...
	function MAIStatusBar.XPBarPet.Think()
		...
		if not MAIIsXPBarVisible() and not MAIIsREPBarVisible() then
			if not InCombatLockdown() then
				if self.pos ~= 1 then
					self.pos = 1
		...
		elseif not MAIIsXPBarVisible() or not MAIIsREPBarVisible() then
			if not InCombatLockdown() then
				if self.pos ~= 2 then
					self.pos = 2
	
					self:SetPoint("TOPLEFT", MAIStatusBar, "TOPLEFT", 0, -MAIStatusBar:GetHeight() / MAIStatusBar:GetBarCount())
				end
			end
		else
			if not InCombatLockdown() then
				if self.pos ~= 3 then
					...
					self:SetPoint("TOPLEFT", MAIStatusBar, "TOPLEFT", 0, -MAIStatusBar:GetHeight() / MAIStatusBar:GetBarCount() * 2)
				end
			end
		end
